...
function ShoppingHome(){
    ...
    const [currentSlide,setCurrentSlide]=useState(0)  // 0 means first index
    const {productList,productDetails}=useSelector(state=> state.shopProducts)
    const [openDetailsDialog,setOpenDetailsDialog]=useState(false)
    const {user}=useSelector(state=> state.auth)  //for handleAddToCart
    ...
    function handleNavigateToListingPage(getCurrentItem,section){
        ...
        const currentFilter={
            ...
            //section we're having should equal to currentItem.id bcz id only is equal to men
        }
    ...
    useEffect(()=>{
        ...
        return ()=>clearInterval(timer) // clear interval everytime
    },[featureImageList])
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {
                            ...
                            productList && productList.length >0 ? productList.map((productItem)=>
                                (<ShoppingProductTile handleGetProductDetails={handleGetProductDetails} product={productItem} handleAddtoCart={handleAddtoCart}/>)):null
                        }
                    </div>
                    </div>
                </section>
