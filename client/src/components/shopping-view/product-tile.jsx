import { brandOptionsMap, categoryOptionsMap } from "@/configure"
...
function ShoppingProductTile({product,handleGetProductDetails,handleAddtoCart}) {
  return (
    ...
    <Card className="w-full max-w-sm mx-auto">
        ...
        <CardFooter>
            {
                product?.totalStock ===0 ? <Button className="w-full bg-black text-white opacity-60 cursor-not-allowed">
                    Out of Stock
                </Button> : <Button onClick={()=>handleAddtoCart(product?._id,product?.totalStock)} className="w-full bg-black text-white">
                    Add to Cart
                </Button>
            }
            </CardFooter>
